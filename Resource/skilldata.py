...
def use_skill(command, target_player, target_enemy_list):
    if len(command) > 1:
        if int(command[1]) <= len(target_player.skill):
            if target_player.energy >= target_player.skill[int(command[1])][0] and target_player.skill[int(command[1])][2] == 0:
                if target_player.skill[int(command[1])][1] == 'draw':
                    target_player.energy -= target_player.skill[int(command[1])][0]
                    draw_card(target_player)
                    target_player.skill[int(command[1])][2] = target_player.skill[int(command[1])][3]
                if target_player.skill[int(command[1])][1] == 'firepunch':
                    if len(command) > 2:
